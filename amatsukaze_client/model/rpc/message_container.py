...
from amatsukaze_client.model.rpc.message import AmatsukazeRPCMessage, from_messages_bytes as amatsukaze_message_from_messages_bytes
...
class AmatsukazeRPCMessageContainer():
    ...
    def __eq__(self, __o: object) -> bool:
        return self.length == __o.length \
            and self.message_type_id == __o.message_type_id \
            and self.__messages_to_bytes() == __o.__messages_to_bytes()

def from_messages_bytes(message_type_id: AmatsukazeRPCMessageTypeId, length: int, messages_bytes: bytes) -> AmatsukazeRPCMessageContainer:
