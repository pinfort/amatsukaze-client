...
class QueueItem:
    ...
    def encode_finish(self) -> datetime:
        if self.encode_start == datetime.min or self.encode_time == timedelta(
            0, 0, 0, 0, 0, 0, 0
        ):
            return datetime.min
    ...
    def is_active(self) -> bool:
        return (
            self.state == QueueState.ENCODING
    ...
    def is_one_seg(self) -> bool:
        return self.image_width <= 320 or self.image_height <= 260

    @property
    def is_separate_hash_required(self) -> bool:
        return (
            self.mode == ProcMode.BATCH
